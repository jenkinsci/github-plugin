...
import com.coravy.hudson.plugins.github.GithubExcludeUsersProperty;
...
public class GitHubPushTrigger extends Trigger<AbstractProject<?, ?>> implements GitHubTrigger {
  ...
  /**
   * Decided whether to fire the build trigger based on who pushed the change.
   * 
   * @param triggeredByUser
   *          The user that pushed the change
   * 
   * @return true if the trigger should fire otherwise return false.
   */
  private boolean triggerForUserCommit(String triggeredByUser) {
    boolean triggerForCommit = true;
    GithubExcludeUsersProperty property = (GithubExcludeUsersProperty)job
      .getProperty(GithubExcludeUsersProperty.class);

    if (property.getExcludeUsersList().contains(triggeredByUser)) {
      triggerForCommit = false;
    }

    return triggerForCommit;
  }
  ...
  public void start(AbstractProject<?, ?> project, boolean newInstance) {
    ...
    if (newInstance && getDescriptor().isManageHook()) {
      // make sure we have hooks installed. do this lazily to avoid blocking the
      // UI thread.
  ...
  private boolean createJenkinsHook(GHRepository repo, URL url) {
    try {
      repo.createHook("jenkins",
        Collections.singletonMap("jenkins_hook_url", url.toExternalForm()), null, true);
      return true;
    } catch (IOException e) {
  ...
  public final class GitHubWebHookPollingAction implements Action {
    ...
    public void writeLogTo(XMLOutput out) throws IOException {
      new AnnotatedLargeText<GitHubWebHookPollingAction>(getLogFile(), Charset.defaultCharset(),
  ...
  public static class DescriptorImpl extends TriggerDescriptor {
    private transient final SequentialExecutionQueue queue = new SequentialExecutionQueue(
    ...
    public URL getHookUrl() throws MalformedURLException {
      return hookUrl != null ? new URL(hookUrl) : new URL(Hudson.getInstance().getRootUrl()
        + GitHubWebHook.get().getUrlName() + '/');
    }
   ...
   */
  public static boolean ALLOW_HOOKURL_OVERRIDE = !Boolean.getBoolean(GitHubPushTrigger.class
    .getName() + ".disableOverride");
