...
public class GitHubCommitNotifier extends Notifier {
        ...
        for (GitHubRepositoryName name : GitHubRepositoryNameContributor.parseAssociatedNames(build.getProject())) {
            for (GHRepository repository : name.resolve()) {
                ...
                repository.createCommitStatus(
                        ...
                        defaultIfEmpty(statusMessage.expandAll(build, listener), status.getMsg()),
                        contextNameFor(build)
                );
            }
        }
    }
    ...
    private static String contextNameFor(@Nonnull AbstractBuild<?, ?> build) {
        GithubProjectProperty ghProp = build.getProject().getProperty(GithubProjectProperty.class);
        if (ghProp != null && isNotBlank(ghProp.getStatusContext())) {
            return ghProp.getStatusContext();
        }
